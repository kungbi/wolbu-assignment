...
class EnrollmentIntegrationTest {
    ...
    void concurrentEnrollment_CapacityLimit() throws Exception {
        // Given
        // 강사 생성
        Member instructor = Member.create("강사", "instructor@example.com", "010-0000-0000", "password", MemberRole.INSTRUCTOR);
        ...
        List<Member> students = IntStream.range(0, 20)
                .mapToObj(i -> Member.create("학생" + i, "student" + i + "@example.com", "010-1234-567" + (i % 10), "password", MemberRole.STUDENT))
                .map(student -> memberRepository.saveAndFlush(student))
                .toList();
        ...
        try {
            List<CompletableFuture<EnrollmentResult>> futures = students.stream()
                    .map(student -> CompletableFuture.supplyAsync(() -> {
                        try {
                            EnrollmentRequest request = new EnrollmentRequest(List.of(savedLecture.getId()));
                            return enrollmentService.enrollInLectures(student.getId(), request);
                        } catch (Exception e) {
                            // 예외 발생 시 실패 결과 반환
                            EnrollmentResult.EnrollmentFailure failure = new EnrollmentResult.EnrollmentFailure(
                                savedLecture.getId(), "인기 강의", "SYSTEM_ERROR", e.getMessage());
                            return new EnrollmentResult(List.of(), List.of(failure));
                        }
                    }, executor))
                    .toList();
            ...
            // Then
            long totalSuccessCount = results.stream()
                    .mapToLong(EnrollmentResult::getSuccessCount)
                    .sum();
            
            long totalFailureCount = results.stream()
                    .mapToLong(EnrollmentResult::getFailureCount)
                    .sum();
    ...
    void duplicateEnrollmentPrevention() {
        // Given
        Member instructor = Member.create("강사", "instructor2@example.com", "010-0000-0001", "password", MemberRole.INSTRUCTOR);
        ...
        // When
        // 첫 번째 신청 (성공해야 함)
        EnrollmentResult firstResult = enrollmentService.enrollInLectures(savedStudent.getId(), request);
        
        // 두 번째 신청 (실패해야 함)
        EnrollmentResult secondResult = enrollmentService.enrollInLectures(savedStudent.getId(), request);
    ...
    void reEnrollmentAfterCancellation() {
        // Given
        Member instructor = Member.create("강사", "instructor3@example.com", "010-0000-0002", "password", MemberRole.INSTRUCTOR);
        Member savedInstructor = memberRepository.saveAndFlush(instructor);
        ...
        // 3. 재신청 (성공해야 함)
        EnrollmentResult reEnrollResult = enrollmentService.enrollInLectures(savedStudent.getId(), request);
