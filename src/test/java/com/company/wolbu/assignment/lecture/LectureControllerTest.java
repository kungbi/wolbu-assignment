...
class LectureControllerTest {
    ...
    void createLecture_instructor_success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/lectures").header("Authorization", "Bearer " + instructorToken)
                        .contentType(MediaType.APPLICATION_JSON).content(requestJson)).andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.id").exists())
                .andExpect(jsonPath("$.data.title").value("내집마련 기초반"))
                .andExpect(jsonPath("$.data.maxCapacity").value(10)).andExpect(jsonPath("$.data.price").value(200000))
    ...
    void createLecture_student_fails() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/lectures").header("Authorization", "Bearer " + studentToken)
    ...
    void createLecture_invalid_token_fails() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/lectures").header("Authorization", "Bearer invalid_token")
    ...
    void createLecture_no_auth_header_fails() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/lectures").contentType(MediaType.APPLICATION_JSON).content(requestJson))
    ...
    void createLecture_validation_fails() throws Exception {
        // Given - 빈 제목
        CreateLectureRequestDto request = new CreateLectureRequestDto("", 10, 200000);
        String requestJson = objectMapper.writeValueAsString(request);
    ...
    void getLecture_success() throws Exception {
        // Given - 먼저 강의를 생성
        CreateLectureRequestDto createRequest = new CreateLectureRequestDto("내집마련 기초반", 10, 200000);
        String createRequestJson = objectMapper.writeValueAsString(createRequest);

        String createResponse = mockMvc.perform(
                        post("/api/lectures").header("Authorization", "Bearer " + instructorToken)
                                .contentType(MediaType.APPLICATION_JSON).content(createRequestJson)).andExpect(status().isOk())
        ...
        // When & Then
        mockMvc.perform(get("/api/lectures/" + lectureId)).andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.id").value(lectureId))
                .andExpect(jsonPath("$.data.title").value("내집마련 기초반"))
                .andExpect(jsonPath("$.data.maxCapacity").value(10)).andExpect(jsonPath("$.data.price").value(200000));
    }
    ...
    void getLecture_not_found_fails() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/lectures/" + nonExistentLectureId)).andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false))
    ...
    void getLectureList_success() throws Exception {
        // Given
        mockMvc.perform(get("/api/lectures")).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true));
    }
}
